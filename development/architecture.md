...
CircleChat 的设计目标是**单进程**运行：前端静态资源由后端内部静态服务托管，后端一个 Node.js 进程同时承载 HTTP API、WebSocket 与文件上传。运行时**零第三方运行时依赖**——除 Node 内建模块（含 `node:sqlite`）外不引入任何第三方运行时 npm 包，所有网络 / 协议能力都是自研。
